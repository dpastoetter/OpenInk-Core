...
- **Single-file legacy (preferred):** The build produces a single IIFE bundle (`openink-legacy-single.js`) with no SystemJS or dynamic imports, **transpiled to ES5** via Babel (target Chrome 44) so old Kindle/Silk engines can parse and run it. Includes `regenerator-runtime` for async/await. When present, `legacy.html` loads only that one script. If the single build was not run, the generator falls back to polyfill + `System.import(entry)`.
